...
resource "null_resource" "apply_k8s_manifests" {
  ...
  provisioner "local-exec" {
    command = "kubectl apply -f \"${abspath("${path.module}/../k8s/volume.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/persistClaim.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/deployment-sql.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/service-sql.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/secret.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/environment.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/deployment.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/service.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/metrics.yaml")}\" && kubectl apply -f \"${abspath("${path.module}/../k8s/hpa.yaml")}\""
  }
}
